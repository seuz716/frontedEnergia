...
function ResultadoPais(pais) {
  const [busqueda, setBusqueda] = useState("");
  const [resultados, setResultados] = useState([]);

  useEffect(() => {
    if (busqueda.length >= 1) {
      PlantasServices.servicioBusquedaPais(busqueda)
  ...
  function handleChangeBarraBusqueda(params) {
    ...
    let paisCentral = params.target.value;
    // console.log( tituloPlanta.length );
    setBusqueda(paisCentral);
  }
  ...
  return (
    ...
    <div>
     ...
     <div>
      <legend>listando Centrales Electricas: </legend> 
      </div>
      <div>
        {" "}
